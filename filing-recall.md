...
0. **Duplicate check**: before writing, check _index.md for an existing
   entry matching this date + topic (and for OneNote ingests, the same
   source page). If a likely match exists, present it and ask — file as
   new, merge, or skip. Never silently create a near-duplicate. On a retry
   after partial failure, resume at the first incomplete step; never
   re-run completed writes.
